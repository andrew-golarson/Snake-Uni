...
void Grid::initTilemap(Grid tilemap[gridSize][gridSize]){
    ...
    for(int i = 0; i < gridSize; ++i){
        for(int j = 0; j < gridSize; ++j){
            tilemap[i][j].x = tileSize * j; // pozycja kolejnych płytek co szerokość płytki
            tilemap[i][j].y = tileSize * i; // pozycja kolejnych rzędów płytek co wysokość płytki
            tilemap[i][j].tile.setSize({tileSize, tileSize});
            if(tileChange){
                tilemap[i][j].tile.setTexture(&darkTex);
                tileChange = !tileChange; // zmiana tekstury przy nastepnej iteracji
            }else{
    ...
    tileChange = !tileChange; // zmianą kolejności tekstur
    }
}